...
For more information on the plugin internals, please check out the source code. All the plugin's code is covered with tests that were placed in a separate staging rails project located at http://github.com/kovyrin/db-charmer-sandbox/tree/master. The project has unit tests for all or at least the most of the parts of plugin's code.
